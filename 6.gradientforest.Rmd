...
2) You will also need gfortran on your computer, which  you can download from here: https://gcc.gnu.org/wiki/GFortran

##How did PC people fix it? It was a syntax error, not a compiling error (I don't think)
...
```{r}
coye_shape=st_read("~/Dropbox/BGP/genoscape_maps/shapefiles/old_NatureServe_shapefiles/WIFL/WIFLrev.shp")
...
Use the code below to look at Cuumulative importance, aka which variants are strongly associated with these top variables. The cumulative importance is plotted separately for all species and averaged over all species. The cumulative importance can be interpreted as a mapping from an environmental gradient on to biological gradient.
```
...
__Which SNP has the greatest importance?__
...
birdgrid=read.table("wiflcurrentRH.csv",header=T,sep=",") %>% filter(BIO4!="-9999") #load your  random points with assocaited env predictors 
...
plot(euc$Long,euc$Lat,pch=".",cex=3,col=colramp)
lines(wifl_shape)
```
...
__How does this look if we plot the second scenario of higher gas emissions? Submit this second plot too. Does it look different than the first plot?__
