...
One reason high covariance among individuals can happen is relatedness. To test relatedness among the indivdiuals, to confirm the removal of those indivdials from subsequent analyses, I use ngsRelate in the terminal.
...
```{r}
meta<-read.xls("data/srs_data/COYE_master_Updated06062022.xlsx",sheet=3) %>% dplyr::select(Sample_data,State) %>% rename(Sample=Sample_data,Region_site=State)

relSC<-read_delim("output_files/ngsRelate_results/COYEP1-4c.merged_gatk.srs_filt.subset.recode.vcf.ngsRelate.res",delim="\t") %>%  
  rename(IndA=inda,IndB=indb) %>% 
  dplyr::select(IndA,IndB,everything())

RegA<-relSC %>% dplyr::select(IndA) %>% rename(Sample=IndA) %>% left_join(meta) %>% rename(RegionA=State)
RegB<-relSC %>% dplyr::select(IndB) %>% rename(Sample=IndB) %>% left_join(meta) %>% rename(RegionB=State)
