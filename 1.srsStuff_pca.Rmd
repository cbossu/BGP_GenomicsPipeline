...
Get allele depths for 4,019,279 sites of 15464410 sites using bcftools
```
bcftools query -f '%CHROM\t%POS[\t%AD]\n' COYEP1-4c.merged_gatk.srs_filt.vcf > COYEP1_P4c.allele_depths.txt
```
...
```{r}
snames <- read_lines("data/test.ind")

AllCOYE<-srs_covar(file="data/COYEP1_4c.allele_depths.subset.txt", sample_names=snames, freq_thresh = 0)
saveRDS(AllCOYE, "data/COYE.srs_filtered.202ind.srs.subset.rds")
AllCOYE <- readRDS("data/COYE.srs_filtered.202ind.srs.subset.rds")
```

Incorporate the meta data
```{r}
meta<-read.xls("data/COYE_master_Updated06062022.xlsx",sheet=3) %>% dplyr::select(Sample_data,State,Location) %>% rename(vcf_name=Sample_data,Region_site=State,Region_pop=Location) %>% filter(vcf_name %in% AllCOYE$sample_names) %>% distinct() %>% filter(Region_site!="NA") %>% arrange(Region_site,vcf_name)
dim(meta)
...
Santa Clara birds should be on there due to relatedness, but are there others that I might not have removed before? Choose the covariance cut off that works for your data.
