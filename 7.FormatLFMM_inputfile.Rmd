...
```{r}
output=LEA::vcf2geno("coye.EAST.plate1.sort.SNP.maf05.miss.clean48indv_noOut.138K.recode.vcf","coye.EAST.RAD.maf05.48ind.138K.geno")
output=LEA::geno2lfmm("coye.EAST.RAD.maf05.48ind.138K.geno","coye.EAST.RAD.maf05.48ind.138K.geno.lfmm")

summary(output)
ind<-read_delim("../LFMM/HETH.keep_noHZ.txt",delim="\t",col_names=F) %>% rename(IID=X1)
dim(ind)
...
impute(project, "HETH.all.clean.hethv0.rm_hets.rmrel.rmcont.geno.lfmm",method = 'mode', K = 4, run = best)
```
